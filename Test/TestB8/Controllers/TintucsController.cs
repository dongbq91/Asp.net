...
namespace TestB8.Controllers
{
    public class TintucsController : Controller
    {
        private DBTin db = new DBTin();

        // GET: Tintucs
        public ActionResult Index(string sorttieude)
        {
            ViewBag.Sapten = String.IsNullOrEmpty(sorttieude) ? "tieu_de" : "";
            var tintucs = db.Tintucs.Select(t => t);
            switch (sorttieude)
            {
                case "tieu_de":
                    tintucs = tintucs.OrderByDescending(s => s.Tieudetin);
                    break;
                default:
                    tintucs = tintucs.OrderBy(s => s.Tieudetin);
                    break;
            }
